...
datetime <- "2025-03-10 125332.675184"
...
for (s in 1:1){
  ...
  write.csv(temp_trim_df, sprintf("draws_trim_TGM_fixed_d_and_SD_v4_%s_agent_%s_init0.csv", datetime, s))
  
  #infer_params <- rbind(infer_params, temp_trim_df)
}


# write.csv(infer_params, paste("draws_trim_TGM_fixed_d_and_SD", paste0(datetime, ".csv")))
