...
generated quantities {
  ...
  for (i in 1:ntrials[1]){ // Use ntrials for agent 1
    ...
    EEA_prior[i] = EEA_given_d_prior[i]; 
    for (task in 1:ntasks){
      EEA_prior[i] = EEA_prior[i] - FA_prior[task] * task_dum[i,task,1] * falsealarm[i,1]; // Include effect of FA trial if relevant
    }
