...
corr_matrix2 <- matrix(c(1, 0, 0,
                         0, 1, 0,
                         0, 0, 1), nrow = 3) # No correlation
FA <- mvrnorm(nagents, mu = FA_means, Sigma = corr_matrix2) # Generate 3 correlated variables with m=0 and sd=1
...
for (s in 1:nagents){
  # draw param values for each of the tasks, in this case randomly from unif distributions
  true_as[s,] <- runif(ntasks, 0.8, 2)
  true_bs[s,] <- runif(ntasks, 0.3, 0.7)
  true_vs[s,] <- runif(ntasks, 0.8, 2)
  true_ts[s,] <- runif(ntasks, 0.2, 0.25)
...
true_params <- data.frame(s = seq(1:nagents), w)
...
write.csv(true_params, paste("TGM_unrelated_params_fixed_d_and_SD_true_params", paste0(datetime, ".csv")))
write.csv(sim_dat, paste("TGM_unrelated_params_fixed_d_and_SD_sim_dat", paste0(datetime, ".csv")))
